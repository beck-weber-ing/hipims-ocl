...
public:
  ...
  virtual void
  updateCellStatistics() = 0; // Update the total number of cells calculation
  virtual void writeOutputs() = 0; // Write output files to disk
  void createStoreBuffers(void **, void **, void **,
                          unsigned char); // Allocates memory and returns
                                          // pointers to the three arrays
  void initialiseMemory();                // Populate cells with default values
  void handleInputData(unsigned long, double, unsigned char,
                       unsigned char); // Handle input data for varying
                                       // state/static cell variables
  void setBedElevation(unsigned long,
                       double); // Sets the bed elevation for a cell
  void setManningCoefficient(unsigned long,
                             double); // Sets the manning coefficient for a cell
  void setStateValue(unsigned long, unsigned char,
                     double); // Sets a state variable
  bool isDoublePrecision() {
    return (ucFloatSize == 8);
  }; // Are we using double-precision?
  double getBedElevation(unsigned long); // Gets the bed elevation for a cell
  double getManningCoefficient(
      unsigned long); // Gets the manning coefficient for a cell
  ...
  CBoundaryMap *getBoundaries() {
    return pBoundaries;
  } // Return the boundary map class
  unsigned int getID() { return uiID; }    // Get the ID number
  void setID(unsigned int i) { uiID = i; } // Set the ID number
  virtual mpiSignalDataProgress
  getDataProgress(); // Fetch some data on this domain's progress

  void setScheme(CScheme *); // Set the scheme running for this domain
  CScheme *getScheme();      // Get the scheme running for this domain
  void
  setDevice(COCLDevice *); // Set the device responsible for running this domain
  COCLDevice *getDevice(); // Get the device responsible for running this domain

#ifdef _WINDLL
  virtual void sendAllToRenderer() {
  }; // Allows the renderer to read off the bed elevations
#endif
...
protected:
  ...
  // Private functions
  unsigned char
  getDataValueCode(char *); // Get a raster dataset code from text description
};
