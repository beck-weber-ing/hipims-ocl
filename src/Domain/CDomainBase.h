...
public:
  ...
  // Public functions
  static CDomainBase *
  createDomain(unsigned char); // Create a new domain of the specified type
  virtual DomainSummary
  getSummary(); // Fetch summary information for this domain
  virtual bool
  configureDomain(XMLElement *); // Configure a domain, loading data etc.
  virtual bool isRemote() { return true; }; // Is this domain on this node?
  virtual unsigned char getType() {
    return model::domainStructureTypes::kStructureInvalid;
  }; // Fetch a type code
  ...
  unsigned int getRollbackLimit() {
    return uiRollbackLimit;
  } // How many iterations before a rollback is required?
  ...
  unsigned int getLinkCount() {
    return links.size();
  } // Get the number of links in this domain
  unsigned int getDependentLinkCount() {
    return dependentLinks.size();
  } // Get the number of dependent links
  CDomainLink *getLink(unsigned int i) { return links[i]; }; // Fetch a link
  CDomainLink *getDependentLink(unsigned int i) {
    return dependentLinks[i];
  };                                      // Fetch a dependent link
  CDomainLink *getLinkFrom(unsigned int); // Fetch a link with a specific domain
  bool sendLinkData();                    // Send link data to other nodes
  bool
  isLinkSetAtTime(double); // Are all this domain's links at the specified time?
  void clearLinks() {
    links.clear();
    dependentLinks.clear();
  }                            // Remove pre-existing links
  void addLink(CDomainLink *); // Add a new link to another domain
  void
  addDependentLink(CDomainLink *); // Add a new dependent link to another domain
  void markLinkStatesInvalid();    // When a rollback is initiated, link data
                                   // becomes invalid
  void setRollbackLimit();         // Automatically identify a rollback limit
  void setRollbackLimit(unsigned int i) {
    uiRollbackLimit = i;
  } // Set the number of iterations before a rollback is required
  virtual unsigned long
  getCellID(unsigned long,
            unsigned long); // Get the cell ID using an X and Y index
  virtual mpiSignalDataProgress getDataProgress() {
    return pDataProgress;
  }; // Fetch some data on this domain's progress
  virtual void setDataProgress(mpiSignalDataProgress a) {
    pDataProgress = a;
