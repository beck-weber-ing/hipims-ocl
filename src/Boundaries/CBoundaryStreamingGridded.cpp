 ...
 */
bool CBoundaryStreamingGridded::setupFromConfig(
    XMLElement *pElement, std::string sBoundarySourceDir) {
  char *cBoundaryType, *cBoundaryName, *cBoundaryMask, *cBoundaryInterval,
      *cBoundaryValue;
  ...
  if (CXMLDataset::isValidFloat(cBoundaryInterval)) {
    ...
    model::doError("Gridded boundary interval is not a valid number.",
  ...
  if (cBoundaryValue == NULL || strcmp(cBoundaryValue, "rain-intensity") == 0) {
    ...
    model::doError("Unrecognised value parameter specified for gridded "
                   "timeseries data. Currently supported are: "
                   "rain-intensity, loss-rate, mass-flux.",
                   model::errorCodes::kLevelWarning);
  }

  // Allocate memory for the array of gridded inputs
  // this->uiTimeseriesLength = static_cast<unsigned
  // int>(ceil(pManager->getSimulationLength() / dInterval)) + 1;
  this->uiTimeseriesLength = static_cast<unsigned int>(floor(
                                 pManager->getSimulationLength() / dInterval)) +
                             1;
  // this->pTimeseries = new
  // CBoundaryStreamingGriddedEntry*[this->uiTimeseriesLength];
  ...
  for (double dTime = 0.0; dTime <= pManager->getSimulationLength();
       dTime += dInterval) {
    ...
    if (!Util::fileExists(sFilename.c_str())) {
      model::doError("Gridded boundary raster missing for " +
                         Util::secondsToTime(dTime) + " with filename '" +
                         sFilename + "'",
                     model::errorCodes::kLevelWarning);
    ...
    if (pTransform == NULL) {
      ...
      pTransform = pRaster->createTransformationForDomain(
          static_cast<CDomainCartesian *>(this->pDomain));
      delete pRaster;
    }
  }
...
void CBoundaryStreamingGridded::prepareBoundary(
    COCLDevice *pDevice, COCLProgram *pProgram, COCLBuffer *pBufferBed,
    COCLBuffer *pBufferManning, COCLBuffer *pBufferTime,
    COCLBuffer *pBufferTimeHydrological, COCLBuffer *pBufferTimestep) {
  ...
  if (pProgram->getFloatForm() == model::floatPrecision::kSingle) {
    ...
    this->pBufferConfiguration =
        new COCLBuffer("Bdy_" + this->sName + "_Conf", pProgram, true, true,
                       sizeof(sConfigurationSP), true);
    std::memcpy(this->pBufferConfiguration->getHostBlock<void *>(),
                &pConfiguration, sizeof(sConfigurationSP));

    this->pBufferValues =
        new COCLBuffer("Bdy_" + this->sName + "_Stream", pProgram, true, true,
                       sizeof(cl_float) * this->pTransform->uiColumns *
                           this->pTransform->uiRows,
                       true);
    /*
    unsigned long ulSize;
    unsigned long ulOffset = 0;
    for (unsigned int i = 0; i < this->uiTimeseriesLength; ++i) {
            void* pGridData = this->pTimeseries[i]->getBufferData(
    model::floatPrecision::kSingle, this->pTransform ); ulSize = sizeof(
    cl_float )* this->pTransform->uiColumns * this->pTransform->uiRows; ulOffset
    += ulSize; std::memcpy(
                    &( ( this->pBufferTimeseries->getHostBlock<cl_uchar*>()
    )[ulOffset] ), pGridData, ulSize
            );
            delete[] pGridData;
    }
    */
    ...
    this->pBufferConfiguration =
        new COCLBuffer("Bdy_" + this->sName + "_Conf", pProgram, true, true,
                       sizeof(sConfigurationDP), true);
    std::memcpy(this->pBufferConfiguration->getHostBlock<void *>(),
                &pConfiguration, sizeof(sConfigurationDP));

    this->pBufferValues =
        new COCLBuffer("Bdy_" + this->sName + "_Series", pProgram, true, true,
                       sizeof(cl_double) * this->pTransform->uiColumns *
                           this->pTransform->uiRows,
                       true);
    /*
    unsigned long ulSize;
    ...
    {
            void* pGridData = this->pTimeseries[i]->getBufferData(
    model::floatPrecision::kDouble, this->pTransform ); ulSize = sizeof(
    cl_double ) * this->pTransform->uiColumns * this->pTransform->uiRows;
            std::memcpy(
  ...
  this->oclKernel->setGlobalSize(ceil(pDomain->getCols() / 8) * 8,
                                 ceil(pDomain->getRows() / 8) * 8);
...
void CBoundaryStreamingGridded::streamBoundary(double dTime) {
  ...
  unsigned int t =
      min(static_cast<unsigned int>(floor(dTime / dTimeseriesInterval)),
          this->uiTimeseriesLength);
  //__private cl_ulong ulTimestep = (cl_ulong)floor( dLclTime /
  // pConfig.TimeseriesInterval ); if ( ulTimestep >= pConfig.TimeseriesEntries
  // )
  // ulTimestep = pConfig.TimeseriesEntries;
  if (this->currentSeriesStep > 0x7FFFFFFF)
    std::cout << "DEBUG SGB bootstrap css: " << this->currentSeriesStep
              << " t: " << t << " dTime: " << dTime
              << " dTimeseriesInterval: " << dTimeseriesInterval << std::endl;
  if (this->currentSeriesStep == t)
    return;
  std::cout << "DEBUG SGB initiate streaming # css: " << this->currentSeriesStep
            << " t: " << t << " dTime: " << dTime
            << " dTimeseriesInterval: " << dTimeseriesInterval << std::endl;
  ...
  this->buffer = new CBoundaryStreamingGriddedEntry(
      dTime, pRaster->createArrayForBoundary(pTransform));
  delete pRaster;

  if (this->singlePrecision) {
    std::cout << "DEBUG SGB SINGLE" << std::endl;
    void *pGridData = this->buffer->getBufferData(
        model::floatPrecision::kSingle, this->pTransform);
    unsigned long size = sizeof(cl_float) * this->pTransform->uiColumns *
                         this->pTransform->uiRows;
    std::memcpy(&((this->pBufferValues->getHostBlock<cl_uchar *>())[0]),
                pGridData, size);
    delete[] pGridData;
  } else {
    std::cout << "DEBUG SGB DOUBLE" << std::endl;
    void *pGridData = this->buffer->getBufferData(
        model::floatPrecision::kDouble, this->pTransform);
    unsigned long size = sizeof(cl_double) * this->pTransform->uiColumns *
                         this->pTransform->uiRows;
    ...
    if (!debug_check)
      std::cout << "DEBUG SGB non-zero data check FAILED" << std::endl;
 ...
 */
CBoundaryStreamingGridded::CBoundaryStreamingGriddedEntry::
    CBoundaryStreamingGriddedEntry(double dTime, double *dValues) {
  this->dTime = dTime;
...
void *CBoundaryStreamingGridded::CBoundaryStreamingGriddedEntry::getBufferData(
    unsigned char ucFloatMode,
  ...
  if (ucFloatMode == model::floatPrecision::kSingle) {
    ...
    for (unsigned long i = 0; i < pTransform->uiColumns * pTransform->uiRows;
         i++)
