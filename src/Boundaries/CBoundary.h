...
enum depthValues {
  ...
  kValueCriticalDepth =
      BOUNDARY_DEPTH_IS_CRITICAL, // Force critical depth based on the discharge
...
enum dischargeValues {
  kValueTotal =
      BOUNDARY_DISCHARGE_IS_DISCHARGE, // Value represents the total discharge
                                       // through the boundary
  kValuePerCell =
      BOUNDARY_DISCHARGE_IS_DISCHARGE, // Value represents the discharge per
                                       // cell through the boundary
  kValueVelocity =
      BOUNDARY_DISCHARGE_IS_VELOCITY, // Value represents a velocity through the
                                      // boundary
  kValueSurging = BOUNDARY_DISCHARGE_IS_VOLUME, // Value represents a depth
                                                // increase in volumetric rate
                                                // terms (e.g. manhole surge)
...
enum lossUnits { kValueRainIntensity = 0, kValueLossRate = 1 };
}
...
public:
  ...
  virtual void prepareBoundary(COCLDevice *, COCLProgram *, COCLBuffer *,
