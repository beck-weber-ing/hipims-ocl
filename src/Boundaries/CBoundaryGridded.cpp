 ...
 */
bool CBoundaryGridded::setupFromConfig(XMLElement *pElement,
                                       std::string sBoundarySourceDir) {
  ...
  if (cBoundaryValue == NULL || strcmp(cBoundaryValue, "rain-intensity") == 0) {
    ...
    model::doError(
        ...
        "Currently supported are: rain-intensity, mass-flux.",
        model::errorCodes::kLevelWarning);
  }
...
void CBoundaryGridded::prepareBoundary(
  ...
  if (pProgram->getFloatForm() == model::floatPrecision::kSingle) {
    ...
    for (unsigned int i = 0; i < this->uiTimeseriesLength; ++i) {
      ...
      ulSize = sizeof(cl_float) * this->pTransform->uiColumns *
    ...
    for (unsigned int i = 0; i < this->uiTimeseriesLength; ++i) {
      ...
      ulSize = sizeof(cl_double) * this->pTransform->uiColumns *
               this->pTransform->uiRows;
      std::memcpy(
          &((this->pBufferTimeseries->getHostBlock<cl_uchar *>())[ulOffset]),
          pGridData, ulSize);
...
void *CBoundaryGridded::CBoundaryGriddedEntry::getBufferData(
