 ...
 *  Constructor
 */
CBoundaryMap::CBoundaryMap(CDomain *pDomain) { this->pDomain = pDomain; }

/*
 *  Destructor
 */
CBoundaryMap::~CBoundaryMap() {
  for (mapBoundaries_t::iterator it = mapBoundaries.begin();
       it != mapBoundaries.end(); it++) {
 ...
 */
void CBoundaryMap::prepareBoundaries(COCLProgram *pProgram,
  ...
  for (mapBoundaries_t::iterator it = mapBoundaries.begin();
       it != mapBoundaries.end(); it++) {
    std::cerr << "DEBUG: prepare boundaries " << it->first
              << " PTR=" << (void *)it->second << std::endl;
    (it->second)
        ->prepareBoundary(pProgram->getDevice(), pProgram, pBufferBed,
                          pBufferManning, pBufferTime, pBufferTimeHydrological,
                          pBufferTimestep);
    std::cerr << "DEBUG: prepared boundaries " << it->first
              << " PTR=" << (void *)it->second << std::endl;
  }
}
...
void CBoundaryMap::applyBoundaries(COCLBuffer *pCellBuffer) {
  for (mapBoundaries_t::iterator it = mapBoundaries.begin();
       it != mapBoundaries.end(); it++) {
    // std::cerr << "DEBUG: apply boundaries " << it->first
    // << " PTR=" << (void *)it->second << std::endl;
    (it->second)->applyBoundary(pCellBuffer);
    // std::cerr << "DEBUG: applied boundaries " << it->first
    // << " PTR=" << (void *)it->second << std::endl;
  }
}
 ...
 */
unsigned int CBoundaryMap::getBoundaryCount() { return mapBoundaries.size(); }
...
bool CBoundaryMap::setupFromConfig(XMLElement *pConfiguration) {
  ...
  sSourceDir = (cSourceDir == NULL || strcmp(cSourceDir, "") == 0
                    ? "./"
                    : (std::string(cSourceDir) + "/"));
  sMapFile = (cMapFile == NULL ? "" : (sSourceDir + std::string(cMapFile)));

  bool mapUsesCoordinates =
      cMapType != NULL && strcmp(cMapType, "coordinates") == 0;
  ...
  while (pTimeSeriesElement != NULL) {
    ...
    if (cBoundaryType != NULL) {
      ...
      if (strcmp(cBoundaryType, "cell") == 0) {
        pNewBoundary =
            static_cast<CBoundary *>(new CBoundaryCell(this->pDomain));
      } else if (strcmp(cBoundaryType, "atmospheric") == 0 ||
                 strcmp(cBoundaryType, "uniform") == 0) {
        pNewBoundary =
            static_cast<CBoundary *>(new CBoundaryUniform(this->pDomain));
      } else if (strcmp(cBoundaryType, "gridded") == 0 ||
                 strcmp(cBoundaryType, "spatially-varying") == 0) {
        pNewBoundary =
            static_cast<CBoundary *>(new CBoundaryGridded(this->pDomain));
      } else if (strcmp(cBoundaryType, "streaming-gridded") == 0) {
        pNewBoundary = static_cast<CBoundary *>(
            new CBoundaryStreamingGridded(this->pDomain));
      } else {
        model::doError("Ignored boundary timeseries of unrecognised type.",
      ...
      if (pNewBoundary == NULL ||
        ...
        model::doError("Encountered an error loading a boundary definition.",
                       model::errorCodes::kLevelWarning);
      } else {
        if (pMapFile != NULL)
      ...
      if (pNewBoundary != NULL) {
        ...
        pManager->log->writeLine("Loaded new boundary condition '" +
                                 pNewBoundary->getName() + "'.");
      } else {
        model::doError(
      ...
      model::doError("Ignored boundary timeseries with no type defined.",
                     model::errorCodes::kLevelWarning);
    }
  ...
  while (pStructureElement != NULL) {
    ...
    if (cBoundaryType != NULL) {
      ...
      if (strcmp(cBoundaryType, "simple-pipe") == 0) {
        pNewBoundary =
            static_cast<CBoundary *>(new CBoundarySimplePipe(this->pDomain));
      } else {
        model::doError("Ignored boundary structure of unrecognised type.",
                       model::errorCodes::kLevelWarning);
      }

      // Configure the new boundary
      if (pNewBoundary == NULL ||
          !pNewBoundary->setupFromConfig(pStructureElement, sSourceDir)) {
        model::doError("Encountered an error loading a structure definition.",
                       model::errorCodes::kLevelWarning);
      }
      ...
      if (pNewBoundary != NULL) {
        ...
        mapBoundaries[pNewBoundary->getName()] = pNewBoundary;
        pManager->log->writeLine("Loaded new structure '" +
                                 pNewBoundary->getName() + "'.");
      } else {
        model::doError(
      ...
      model::doError("Ignored structure with no type defined.",
                     model::errorCodes::kLevelWarning);
    }
...
void CBoundaryMap::applyDomainModifications() {
          ...
          i++)
  {
          ...
          // Reflective cells must be surrounded by disabled cells in all
  directions bar
          // the one they are reflecting, otherwise mass will be distorted.
          //if ( this->pBoundaries[ this->pRelations[ i ].uiTimeseries
  ]->getType() ==
          //	 model::boundaries::types::kBndyTypeReflective )
          //{
          // Disable the boundary cell itself
          pDomain->setStateValue(
                  pDomain->getCellID(this->pRelations[i].fTargetX,
  this->pRelations[i].fTargetY),
  ...
  */
  ...
  pDomain->imposeBoundaryModification(
      CDomainCartesian::kEdgeN,
      this->ucBoundaryTreatment[CDomainCartesian::kEdgeN]);
  pDomain->imposeBoundaryModification(
      CDomainCartesian::kEdgeE,
      this->ucBoundaryTreatment[CDomainCartesian::kEdgeE]);
  pDomain->imposeBoundaryModification(
      CDomainCartesian::kEdgeS,
      this->ucBoundaryTreatment[CDomainCartesian::kEdgeS]);
  pDomain->imposeBoundaryModification(
      CDomainCartesian::kEdgeW,
      this->ucBoundaryTreatment[CDomainCartesian::kEdgeW]);
}
